...
export default function AllJobs() {
  ...
  const jobsBeforeCategoryFilter = useMemo(() => {
    let result = [...allJobs];
    ...
    return result;
  }, [allJobs, jobType, search, statusFilter]);

  const categoryOptions = useMemo(() => {
    const fetchedCategories = [];
    const seenCatNames = new Set();
    jobsBeforeCategoryFilter.forEach((job) => {
      // Only include category if explicitly assigned in the raw API response
      const rawCategory = job.raw?.category_name || job.raw?.categoryName;
      if (!rawCategory) return;

      const name = String(rawCategory).trim();
      const lowerName = name.toLowerCase();
      if (!name || seenCatNames.has(lowerName)) return;
      seenCatNames.add(lowerName);
      fetchedCategories.push(name);
    });
    return fetchedCategories.sort((a, b) => a.localeCompare(b));
  }, [jobsBeforeCategoryFilter]);

  const filteredAndSortedJobs = useMemo(() => {
    let result = [...jobsBeforeCategoryFilter];

    // Filter by category
    if (selectedCategory) {
      result = result.filter((j) => (j.category || "General").trim().toLowerCase() === selectedCategory.toLowerCase());
    }
    ...
    if (sortBy === "newest" || sortBy === "oldest") {
      ...
      result.sort((a, b) => {
        const dateA = new Date(a.createdAt || a.postedDateRaw || 0);
        const dateB = new Date(b.createdAt || b.postedDateRaw || 0);
    ...
    return result;
  }, [jobsBeforeCategoryFilter, selectedCategory, sortBy]);
  ...
  const hasFilters = Boolean(search || selectedCategory || statusFilter !== "active" || sortBy !== "newest" || jobType !== "jobs");
