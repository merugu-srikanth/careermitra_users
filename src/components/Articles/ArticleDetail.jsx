...
function processContent(html) {
  ...
  processed = processed.replace(/<table([^>]*)>/gi, '<div class="table-responsive"><table$1>').replace(/<\/table>/gi, '</table></div>');
...
const RelatedCard = ({ article }) => (
  <Link href={buildArticleUrl(article)} className="group flex gap-3 items-start py-3 border-b border-gray-100 last:border-0">
...
export default function ArticleDetail() {
  ...
  const { parentSlug, childSlug: paramChildSlug, slug: paramSlug, articleSlug } = params;
  const childSlug = paramChildSlug || (articleSlug ? paramSlug : undefined);
