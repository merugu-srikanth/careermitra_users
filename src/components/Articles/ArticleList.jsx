...
const ArticleCard = ({ article }) => {
  ...
  return (
    <Link href={buildArticleUrl(article)} className="group block h-full">
...
export default function ArticleList() {
  ...
  const { parentSlug: pathParent, childSlug: pathChild, slug: pathSlug } = params;
          ...
          <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8">
            ...
            <nav className="flex items-center gap-1.5 text-xs text-gray-400 mb-3 flex-wrap">
              ...
              {activeParent ? (
                <>
                  <span>›</span>
                  <Link href={`/${parentSlugParam}`} className="hover:text-orange-500 transition-colors">{activeParent.name}</Link>
                </>
              ) : (
                <>
                  <span>›</span>
