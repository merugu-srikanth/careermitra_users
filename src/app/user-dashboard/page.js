                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between pt-2.5 border-t border-orange-50">
                    <div className="flex items-center gap-2 flex-wrap text-xs text-slate-400">
                      ...
                      {(() => {
                        const statusText = getDeadlineStatusText(job.application_deadline);
                        if (!statusText) return null;
                        const isAlert = String(statusText).toLowerCase().includes("expires");
                        return (
                          <span
                            className={`inline-flex w-fit items-center rounded-full px-2 py-0.5 text-[10px] font-bold border ${isAlert
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError("");
        // Commented out due to 404 Not Found on production backend
        // const res = await axios.get(`${API_BASE}/marketing-events/my-events`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    (async () => {
      try {
        ...
        setProfile({
          ...
          education: flattenEducation(d.education),
