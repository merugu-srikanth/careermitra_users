...
const JobTableSkeleton = () => (
  <div className="overflow-x-auto rounded-2xl border border-gray-200 bg-white shadow-sm animate-pulse">
                ...
                <tr key={job.id} className="hover:bg-orange-50/30 transition-colors duration-150 group">
                  ...
                  <td className="px-2 py-3 border border-amber-300 align-top">
                    <div className="text-xs text-gray-600 leading-snug">
                      {job.qualifications && job.qualifications.length > 45
                        ? `${job.qualifications.substring(0, 45)}...`
                        : (job.qualifications || "-")}
                    </div>
                    {job.qualifications && job.qualifications.length > 45 && (
                      <button
                  ...
                  <td className="px-2 py-3 border border-amber-300 align-top">
                    <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium whitespace-normal wrap-break-word leading-snug">
                      {job.category}
                    </span>
                  </td>
                  <td className="px-2 py-3 border border-amber-300 align-top">
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        const counts = { jobs: 0, internship: 0, skillup: 0 };
        await Promise.all(
          types.map(async (type) => {
            const url = `https://www.careermitra.in/api/jobs?page=1&limit=1&job_type=${type}${search ? `&search=${encodeURIComponent(search)}` : ""}`;
            const res = await fetch(url);
            const json = await res.json();
            if (json.success) {
              counts[type] = json.data?.pagination?.total ?? 0;
            }
          })
        );
        setTabCounts(counts);
  ...
  useEffect(() => {
    const fetchPageJobs = async () => {
      ...
      try {
        ...
        const json = await res.json();
        if (json.success) {
          const mapped = (json.data?.jobs || []).map(mapUnifiedJob);
          setJobs(mapped);
          setTotalItems(json.data?.pagination?.total ?? mapped.length);
        } else {
